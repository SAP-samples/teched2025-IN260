...
This exercise demonstrates how to configure and enhance the 02.) Delivery Notification - Outbound process in your TPA (Trading Partner Agreement) . This hands-on exercise explores the powerful capabilities of pre-transformation within a Mapping Guideline (MAG). You will learn how to handle duplicates, apply sorting mechanisms, and compute final values based on the restructured order at the target side. These operations, such as target sorting and target computation, are enabled and activated through a post-processing flow configured in a Trading Partner Agreement (TPA).

### What You Will Do
- Implement pre-transformation rules for sorting and restructuring data.
- Learn how to manage duplicate entries effectively
- Apply post-processing flows to finalize target payloads, such as for sorting to restructure data and compute final values

### Prerequisites
- The created and activated TPA from at least Exercise 03.
- The enabled and running Bruno test collection
- The available test data in a Visual Studio Code workspace. 

## Step 01 - Copy Trading Partner MIG
Due to time constraints, you can now create a copy of an already prepared Trading Partner MIG, which is required for making the delivery notification mapping. For this purpose, do the following steps:
...
- (c) In the new window of the copied Trading Partner MIG, enable edit mode and click on the tab **Overview**.
...
For the creation of the overlay, it is now necessary to create an Overlay MAG so that specific delivery notification-related parts, such as pre-transformation and indexing for sorting, can be incorporated. The code value mappings are accordingly. Therefore, you should make the Overlay MAG for the second BTA (Business Transaction Activity) **02.) Delivery Notification - Outbound** according to the following steps:
...
To perform a correct sorting according to the finally requested sorted structure required by the trading partner in its UN/EDIFACT DESADV interchange payload, you should copy the details of the order line items underneath the corresponding packages used for delivery. For this purpose, you have to create a pre-transformation within your Overlay-MAG according to the following steps: 
- (a) Go to the opened Overlay MAG in edit mode to the **Overlay** tab and 
...
Define the rule for copying the line item details of E1EDL24 to each corresponding handling unit E1EDL44 with qualifier VELIN = "3" via the following steps:
- (a) Collapse the whole structure starting from root node **DELVRY07** (Delivery Interface) and expand it so that you see the direct child group nodes **EDI_DC40** and **E1EDL20**.
...
- (c) Expand the group node **E1EDL24** (Delivery Items) so that you can see all descendants and drag and drop this group node to the **Referenced node**.
...
## Step 05 - Duplicate the Handling Units at the Target Side
Initially, only the Delivery Header parts are mapped to a UN/EDIFACT segment group, SG10, which covers, in this case, the packaging details of the shipment. You also need further packaging details for pallets included in the shipment and for any additional handling units on the pallets. For this purpose, you can do a duplicate by following these steps:
- (a) tab change **Main Transformation**.
- (b) You should now see the additional group **E1EDL24Copy** which was created by step (04). However, the required data from this group has not been mapped yet.
- (c) For this purpose, you need further target groups, which can be added by clicking on **SG 10 ... --> Duplicate**.
- (d) In the pop-up window **Duplicate Node "SG10** clik on button **Create** for
- (e) Adding the new duplicated group where you should enter the name `Pallets`.
- (f) Repeat it for getting another duplicated group where you should enter the name `Units`.
- (g) Once you click on the button **Apply**, you should see these duplicated groups on the target structure side.

![IN260 Figure 06.05](assets/IN260_06.05.png)

## Step 06 - Add Constants and Mapping Elements
Once you're ready with the duplicates, you can now add the following constants and mapping elements in the Overlay MAG according to the following steps:
- (a) **Group Mapping Elements** and **Pass-Through Mapping Elements**: Drag and drop the line from the source node to the required target node (see below).
  ***Remark:** Group Mapping Elements are only required if the source and the corresponding target group node have a maximum. Occurrence higher than 1.
- (b) **Constants**: Click on the action button at the target leaf node (see below), (c) select the action button **[...]** and (d) select **Set Constant**.
- (e) **Mapping Elements with Mapping Functions:** Drag and drop the line from the source node to the required target node, and (f) enter in tab **Functions** the given mapping function as shown below.
- (g) **Leaf Mapping Elements with Code Value Mappings:** Drag and drop the line from the source node to the required target node, (g) select **Add --> All remain source values**, and (i) select all the corresponding target code values as shown in the list below. 
...
***Remark:*** If you like, you can simulate in the MAG before proceeding with the next step.

![IN260 Figure 06.07](assets/IN260_06.07.png)

## Step 08 - Run Comparison Test
Do a test run and compare the results with the expected payloads. There will be mismatches because the target payload is not sorted to the predicted structure.
The following steps can realize the comparison test:
...
To obtain the correct order at the target payload side, you require the post-processing flow with the specific target sorting script, which is already prepared in the given Integration Package: **TechEd 2025 - IN260 - Post Processing Flows**. To get it, you should do the following steps: 
...
- (c) Enter in the pop-up window the **Name:** `Post-Processing - UserXX - 02 - Delivery Notification - Outbound UN-EDIFACT`, and (d) click on the button **Copy**.
- (e) You'll see the copied integration flow in where you should (f) select action item **Configure**, 
- (g) Change in the pop-up window, the **UserID**, the value **XX** in the name to your ID, and (h) click on the button **Deploy**. 
...
- (b) Click on the target **Interchange** step at the receiver side.
- (c) In the details panel, enable the **Customized Post-Processing**.
- (d) Add in the field **Process Direct Address** the address: 
...
Run the same test as described in step 08 and compare the results with the expected payloads. If you don't see any mismatches, then all the creations and changes you made in your Overlay MAG appear to be correct, and the post-processing is running sufficiently.
